...
def contrast_table(patterNum, table):
	contrastTable = table
	for i in range(0, len(patterNum)):
		for j in range(0, len(contrastTable)):
			if 2**j == patterNum[i]:
				for key in contrastTable[j]:
					contrastTable[j][key] = 'x'
...
def cellular_automaton(string, pattern, n):
	...
	patterNum = power_sum(pattern) # get power of two elements
	table = contrast_table(patterNum, lookup_table) # update the lookup table

	for unused in range(n):
		result = ''
		for i in range(0, len(string)):
			if i+1 == len(string):
				char = string[i-1] + string[i] + string[0]
			else:
				char = string[i-1] + string[i] + string[i+1]
			for entry in table:
				if char in entry:
					result += entry[char]
		string = result
...
#>>> .xxx....
#[{'...': 'x'}, {'..x': '.'}, {'.x.': 'x'}, {'.xx': 'x'}, {'x..': 'x'}, {'x.x': 'x'}, {'xx.': 'x'}, {'xxx': '.'}]
...
# Udacity Code
def cellular_automation_submit(string, patter_number, generations):
	patterns = {}
	...
	for i in range(7,-1,-1):
		if patter_number/(2**i) == 1:
			patterns[pattern_list[i]] = 'x'
			patter_number = patter_number - 2**i
		else:
	...
	for j in range(0, generations):
		new_string = ''
		for i in range(0, n):
			pattern = string[i-1] + string[i] + string[(i+1)%n] # Using % to get override element, good skill
...
# Testing 2
print 'Testing 2'
print cellular_automation_submit('...x....', 125, 5)
#>>> ...xxx.x
print cellular_automation_submit('...x....', 125, 6)
#>>> xx.x.xxx
print cellular_automation_submit('...x....', 125, 7)
#>>> .xxxxx..
print cellular_automation_submit('...x....', 125, 8)
#>>> .x...xxx
print cellular_automation_submit('...x....', 125, 9)
#>>> xxxx.x.x
print cellular_automation_submit('...x....', 125, 10)