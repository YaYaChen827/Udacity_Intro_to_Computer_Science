...
# Submit Code
def is_reciprocal(graph, source, destination, depth):
    if depth == 0:
        if destination == source: # a->a
            return True
        return False

    for node in graph[destination]: # a->a or b or c
        if is_reciprocal(graph, source, node, depth-1):
            return True
    return False
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    # Note that, we judge each node with reciprocal graph in page
