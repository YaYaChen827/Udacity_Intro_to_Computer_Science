...
def involved(courses, person):
  offered = {}
  # Note that the courses[hexa]'s next level is shown courses[hexa][name].
  # Using this concept, we can solve this problem
  for hexa in courses:
    for name in courses[hexa]:
      for teacher in courses[hexa][name]:
        if person == courses[hexa][name][teacher]:
          # Seting output expression
          if hexa in offered:
            offered[hexa].append(name)
          else:
            offered[hexa] = [name]
  return offered
